...
from bin.rpg.friendship.config import *
from bin.rpg.rpgsql import limit, make_friend,get_money_info,cost_money,get_friend_level,get_friend_exp,edit_friend_exp
from config.color import *
from config.emoji import BACK,PENCIL,GREEN_HEART
...
class confession_check_button(View):
    def __init__(self):
...
class make_friendship(View):
    ...
    async def accept_friend_callback(self, interaction: Interaction, button: Button):
        if interaction.user.id == self.author.id:return await interaction.response.send_message(ephemeral=True, embed=getembed(f"{BACK} | 強行交友不好喔！", "", RED))
        elif interaction.user.id == self.to.id:
            await interaction.response.edit_message(embed=getembed(f"{GREEN_HEART} | {self.author.name} 與 {self.to.name} 成為了朋友", "", GREEN), view=None)
            return make_friend(self.author.id, self.to.id)
        else:return await interaction.response.send_message(ephemeral=True, embed=getembed(f"{BACK} | 怎麼可以去干涉別人感情呢", "", RED))
    ...
    async def refuse_friend_callback(self, interaction: Interaction, buttin: Button):
        if interaction.user.id == self.author.id:return await interaction.response.send_message(ephemeral=True, embed=getembed(f"{BACK} | 問心酸的？", "", RED))
        elif interaction.user.id == self.to.id:
            return await interaction.response.edit_message(embed=getembed(f"{self.to.name} 拒絕當 {self.author.name} 的朋友", "", BLACK), view=None)
        else: return await interaction.response.send_message(ephemeral=True, embed=getembed(f"{BACK} | 怎麼可以去干涉別人感情呢", "", RED))
